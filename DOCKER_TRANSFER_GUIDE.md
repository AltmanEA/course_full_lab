...
### 3.4. Использование готовых образов без сборки

По умолчанию `docker-compose up` запускает сборку образов, если в `docker-compose.yml` указан раздел `build`. Чтобы избежать сборки и использовать уже загруженные образы, вы можете:

1. **Использовать специальный файл `docker-compose.transfer.yml`**, который вместо `build:` указывает `image:` с именами загруженных образов. После загрузки образов выполните:

   ```bash
   docker-compose -f docker-compose.transfer.yml up -d
   ```

   Этот файл уже включён в проект и копируется в папку `backup` при резервном копировании.

2. **Использовать флаг `--no-build`** с оригинальным `docker-compose.yml`:

   ```bash
   docker-compose up -d --no-build
   ```

   Однако если образы отсутствуют, контейнеры не запустятся.

Рекомендуется использовать первый способ, так как он гарантирует, что сборка не начнётся даже при случайном изменении контекста.
